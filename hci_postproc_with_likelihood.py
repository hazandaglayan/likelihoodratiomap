...
"""
We modified the module with the HCI<post-processing algorithms> classes in VIP-HCI package.
"""

__author__ = 'Carlos Alberto Gomez Gonzalez, Ralf Farkas, modified by Hazan Daglayan'
...
from vip_hci.psfsub import pca, llsg, median_sub, xloci, pca_annular
from vip_hci.config.utils_conf import algo_calculates_decorator as calculates
...
class HCIPca(HCIPostProcAlgo):
    ...
    @calculates("snr_fluxmap","likelihoodmap")
    def make_likelihoodmap(self, plot=False, verbose=False, likelihood_norm=1):
        """
        Calculate a likelihood map from ``self.residual_cube_list``.
        likelihoodmap(residual_cube_list, angle_list, psfn, planet_position_list, fwhm=4, plot=False):

        Parameters
        ----------
        plot : bool, optional
            If True plots the likelihood map. False by default.
        fwhm : FWHM 
        verbose : bool, optional
            Whether to print timing or not.
	likelihood_norm : int
	    1 for Laplace, 2 for Gaussian

        """

        cy, cx = vip.var.frame_center(self.dataset.cube)
        pixels = pixels_in_annulus(self.dataset.cube.shape[1:], (cy, cx), self.inner, self.outer)
        residual_cube_list = self.cube_residuals
        angle_list = self.dataset.angles
        psfn = self.dataset.psfn
        fwhm = self.dataset.fwhm
        
        fluxmap, self.likelihoodmap = likelihoodmap(residual_cube_list, angle_list, psfn, pixels, fwhm=fwhm, plot=plot, norm=likelihood_norm, n_jobs=8)
        self.snr_fluxmap = vip.metrics.snrmap(fluxmap, self.dataset.fwhm, verbose=False, plot=plot)

        
class HCIAnnPca(HCIPostProcAlgo):
    """ HCI PCA algorithm.
    ...
    """
    def __init__(self, dataset=None, ncomp=1, ncomp2=1, svd_mode='lapack', scaling=None,
                 adimsdi='double', mask_center_px=None, source_xy=None, scale_list= None,
                 delta_rot=1, delta_sep=(0.1, 1), imlib='opencv', interpolation='lanczos4',
                 collapse='median', check_mem=True, crop_ifs=True, verbose=True, inner=3, outer=13, 
                 asize=4, radius_int=0, n_segments=1):

        super(HCIAnnPca, self).__init__(locals())
    ...
    def run(self, dataset=None, nproc=1, verbose=True, debug=False):
        ...
        res = pca_annular(dataset.cube, dataset.angles, 
                  ncomp=self.ncomp, svd_mode=self.svd_mode, asize = dataset.fwhm,  fwhm=dataset.fwhm,
                  full_output=True, verbose=True)
        

        cuberes, cuberesder, frame = res 
        self.cube_residuals = cuberes
        self.cube_residuals_der = cuberesder
        self.frame_final = frame
    ...
    def make_likelihoodmap(self, plot=False, verbose=False, likelihood_norm=1):
        """
        Calculate a likelihood map from ``self.residual_cube_list``.
        likelihoodmap(residual_cube_list, angle_list, psfn, planet_position_list, fwhm=4, prad=1, plot=False):
        ...
        plot : bool, optional
            If True plots the likelihood map. False by default.
        fwhm : FWHM 
        prad : disk size, radius of the removed part in each frame
        verbose: bool, optional
            Whether to print timing or not.
	likelihood_norm : int
	    1 for Laplace, 2 for Gaussian
